...
class GeodesicTest(unittest.TestCase):
    ...
    def test_inverse(self):
        ...
        for l in GeodesicTest.testcases:
            lat1, lon1, azi1, lat2, lon2, azi2 = np.deg2rad(l[:6])
            s12, a12, m12, M12, M21, S12 = l[6:]
            inv = wgs84.inverse(lat1, lon1, lat2, lon2)
            s122, faz, baz, a122= inv[:4]
            baz = np.mod(baz+np.pi*2, np.pi*2) - np.pi
            # self.assertAlmostEqual(lon2, inv["lon2"], delta=1e-13)
            self.assertAlmostEqual(azi1, faz, delta=1e-10)
            self.assertAlmostEqual(azi2, baz, delta=1e-10)
            self.assertAlmostEqual(s12, s122, delta=1e-3)
            self.assertAlmostEqual(a12, np.rad2deg(a122), delta=1e-7)
    ...
    def test_direct(self):
        ...
        for l in GeodesicTest.testcases:
            ...
            s12, a12, m12, M12, M21, S12 = l[6:]
            lat22, lon22, az22 = wgs84.direct(lat1, lon1, azi1, s12)

            self.assertAlmostEqual(lat2, lat22, delta=1e-10)
            self.assertAlmostEqual(lon2, lon22, delta=1e-10)
            self.assertAlmostEqual(azi2, az22-np.pi, delta=1e-10)
...
class GeodSolveTest(unittest.TestCase):
    ...
    def test_GeodSolve0(self):
        ...
        s12, azi1, azi2 = wgs84.inverse(lat1, lon1, lat2, lon2)[:3]
        azi2 = np.mod(azi2+np.pi*2, np.pi*2) - np.pi
        self.assertAlmostEqual(np.rad2deg(azi1), 53.47022, delta=0.5e-5)
        self.assertAlmostEqual(np.rad2deg(azi2), 111.59367, delta=0.5e-5)
        self.assertAlmostEqual(s12, 5853226, delta=0.5)

    def test_GeodSolve1(self):
        lat1, lon1, az1 = np.deg2rad([40.63972222, -73.77888889, 53.5])
        dir = wgs84.direct(lat1, lon1, az1, 5850e3)

        lat2, lon2, az2 = np.rad2deg(dir)
        az2 = np.mod(az2+360, 360)-180
        self.assertAlmostEqual(lat2, 49.01467, delta=0.5e-5)
        self.assertAlmostEqual(lon2, 2.56106, delta=0.5e-5)
        self.assertAlmostEqual(az2, 111.62947, delta=0.5e-5)

    def test_GeodSolve2(self):
        # Check fix for antipodal prolate bug found 2010-09-04
        geod = Geodesic(6.4e6, -1/150.0)
        lat1, lon1, lat2, lon2 = np.deg2rad((0.07476, 0, -0.07476, 180))
        s12, az1, az2 = geod.inverse(lat1, lon1, lat2, lon2)[:3]
        az1, az2 = np.rad2deg((az1, az2))
        az2 = np.mod(az2+360, 360)-180
        self.assertAlmostEqual(az1, 90.00078, delta=0.5e-5)
        self.assertAlmostEqual(az2, 90.00078, delta=0.5e-5)
        self.assertAlmostEqual(s12, 20106193, delta=0.5)

        lat1, lon1, lat2, lon2 = np.deg2rad((0.1, 0, -0.1, 180))
        s12, az1, az2 = geod.inverse(lat1, lon1, lat2, lon2)[:3]
        az1, az2 = np.rad2deg((az1, az2))
        az2 = np.mod(az2+360, 360)-180
        self.assertAlmostEqual(az1, 90.00105, delta=0.5e-5)
        self.assertAlmostEqual(az2, 90.00105, delta=0.5e-5)
        self.assertAlmostEqual(s12, 20106193, delta=0.5)
    ...
    def test_GeodSolve4(self):
        ...
        self.assertAlmostEqual(s12, 0.072, delta=0.5e-3)

    def test_GeodSolve5(self):
        # Check fix for point2=pole bug found 2010-05-03
        lat1, lon1, az1 = np.deg2rad([0.01777745589997, 30, 0])
        dir = wgs84.direct(lat1, lon1, az1, 10e6)
        lat2, lon2, az2 = np.rad2deg(dir)
        az2 = np.mod(az2+360, 360)-180

        #dir = wgs84.direct(0.01777745589997, 30, 0, 10e6)
        self.assertAlmostEqual(lat2, 90, delta=0.5e-5)
        if lon2 < 0:
            self.assertAlmostEqual(lon2, -150, delta=0.5e-5)
            self.assertAlmostEqual(az2, -180, delta=0.5e-5)
        else:
            self.assertAlmostEqual(lon2, 30, delta=0.5e-5)
            self.assertAlmostEqual(az2, 0, delta=0.5e-5)
    ...
    def test_GeodSolve9(self):
        ...
        s12, az1, az2 = wgs84.inverse(lat1, lon1, lat2, lon2)[:3]
        self.assertAlmostEqual(s12, 19993558.287, delta=0.5e-3)
    ...
    def test_GeodSolve10(self):
        ...
        self.assertAlmostEqual(s12, 19991596.095, delta=0.5e-3)
    ...
    def test_GeodSolve11(self):
        ...
        self.assertAlmostEqual(s12, 19989144.774, delta=0.5e-3)
    ...
    def test_GeodSolve12(self):
        ...
        s12, az1, az2 = geod.inverse(0, 0, np.deg2rad(-10), np.deg2rad(160))[:3]
        az1, az2 = np.rad2deg((az1, az2))
        self.assertAlmostEqual(az1, 120.27, delta=1e-2)
        self.assertAlmostEqual(az2, 105.15, delta=1e-2)
        self.assertAlmostEqual(s12, 266.7, delta=1e-1)

    def test_GeodSolve14(self):
        # Check fix for inverse ignoring lon12 = nan
        s12, az1, az2 = wgs84.inverse(0, 0, 1, np.nan)[:3]
        self.assertTrue(np.isnan(az1))
        self.assertTrue(np.isnan(az2))
        self.assertTrue(np.isnan(s12))
...
#
#         dir = wgs84.direct(40, -75, -10, 2e7)
#         self.assertAlmostEqual(lat2, -39, delta=1)
#         self.assertAlmostEqual(lon2, -254, delta=1)
#         self.assertAlmostEqual(az2, -170, delta=1)
...
#         dir = wgs84.direct(40, -75, -10, 2e7)
#         self.assertAlmostEqual(lat2, -39, delta=1)
#         self.assertAlmostEqual(lon2, 105, delta=1)
#         self.assertAlmostEqual(az2, -170, delta=1)
...
#         dir = wgs84.inverse(0, 539, 0, 181)
#         self.assertAlmostEqual(dir["lon1"], 179, delta=1e-10)
#         self.assertAlmostEqual(lon2, -179, delta=1e-10)
...
#         self.assertAlmostEqual(lon2, 541, delta=1e-10)
#         self.assertAlmostEqual(dir["s12"], 222639, delta=0.5)
#
    def test_GeodSolve33(self):
        ...
        s12, az1, az2 = wgs84.inverse(0, 0, 0, np.deg2rad(179))[:3]
        az1, az2 = np.rad2deg([az1, az2])
        self.assertAlmostEqual(az1, 90.00000, delta=0.5e-5)
        self.assertAlmostEqual(az2, -90.00000, delta=0.5e-5)
        self.assertAlmostEqual(s12, 19926189, delta=0.5)
        s12, az1, az2 = wgs84.inverse(0, 0, 0, np.deg2rad(179.5))[:3]
        az1, az2 = np.rad2deg([az1, az2])
        self.assertAlmostEqual(az1, 55.96650, delta=0.5e-4)
        self.assertAlmostEqual(az2, -55.96649514, delta=0.5e-5)
        self.assertAlmostEqual(s12, 19980862, delta=0.5)
        s12, az1, az2 = wgs84.inverse(0, 0, 0, 180)[:3]
        az1, az2 = np.rad2deg([az1, az2])
        self.assertAlmostEqual(az1, 0.00000, delta=0.5e-5)
        self.assertAlmostEqual(az2, -180.00000, delta=0.5e-5)
        self.assertAlmostEqual(s12, 20003931, delta=0.5)
        s12, az1, az2 = wgs84.inverse(0, 0, np.deg2rad(1), np.deg2rad(180))
        az1, az2 = np.rad2deg([az1, az2])
        self.assertAlmostEqual(az1, 0.00000, delta=0.5e-5)
        self.assertAlmostEqual(az2, -180.00000, delta=0.5e-5)
        self.assertAlmostEqual(s12, 19893357, delta=0.5)
        geod = Geodesic(6.4e6, 0)
        s12, az1, az2 = geod.inverse(0, 0, 0, np.deg2rad(179))[:3]
        az1, az2 = np.rad2deg([az1, az2])
        self.assertAlmostEqual(az1, 90.00000, delta=0.5e-5)
        self.assertAlmostEqual(az2, 90.00000, delta=0.5e-5)
        self.assertAlmostEqual(s12, 19994492, delta=0.5)
        s12, az1, az2 = geod.inverse(0, 0, 0, np.pi)[:3]
        az1, az2 = np.rad2deg([az1, az2])
        self.assertAlmostEqual(az1, 0.00000, delta=0.5e-5)
        self.assertAlmostEqual(az2, -180.00000, delta=0.5e-5)
        self.assertAlmostEqual(s12, 20106193, delta=0.5)
        s12, az1, az2 = geod.inverse(0, 0, np.deg2rad(1), np.pi)[:3]
        az1, az2 = np.rad2deg([az1, az2])
        self.assertAlmostEqual(az1, 0.00000, delta=0.5e-5)
        self.assertAlmostEqual(az2, -180.00000, delta=0.5e-5)
        self.assertAlmostEqual(s12, 19994492, delta=0.5)
        ...
        self.assertAlmostEqual(az2, 90.00000, delta=0.5e-5)
        self.assertAlmostEqual(s12, 20106193, delta=0.5)
        s12, az1, az2 = geod.inverse(0, 0, np.deg2rad(0.5), np.pi)[:3]
        az1, az2 = np.rad2deg([az1, az2])
        self.assertAlmostEqual(az1, 33.02493, delta=0.5e-5)
        self.assertAlmostEqual(az2, 146.97364, delta=0.5e-5)
        self.assertAlmostEqual(s12, 20082617, delta=0.5)
        s12, az1, az2 = geod.Inverse(0, 0, np.deg2rad(1), np.pi)[:3]
        az1, az2 = np.rad2deg([az1, az2])
        self.assertAlmostEqual(az1, 0.00000, delta=0.5e-5)
        self.assertAlmostEqual(az2, -180.00000, delta=0.5e-5)
        self.assertAlmostEqual(s12, 20027270, delta=0.5);
#
#     def test_GeodSolve55(self):
#         # Check fix for nan + point on equator or pole not returning all nans in
#         # Geodesic::Inverse, found 2015-09-23.
#         inv = wgs84.inverse(np.nan, 0, 0, 90)
#         self.assertTrue(np.isnan(az1))
#         self.assertTrue(np.isnan(az2))
#         self.assertTrue(np.isnan(s12))
#         inv = wgs84.inverse(np.nan, 0, 90, 9)
#         self.assertTrue(np.isnan(az1))
#         self.assertTrue(np.isnan(az2))
#         self.assertTrue(np.isnan(s12))
