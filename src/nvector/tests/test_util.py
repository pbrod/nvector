...
@pytest.mark.parametrize(
    "names, datum",
    [
        (("airy1858", 1), Ellipsoid(a=6377563.3960, f=1.0 / 299.3249646, name="Airy 1858")),
        (("airymodified", 2), Ellipsoid(a=6377340.189, f=1.0 / 299.3249646, name="Airy Modified")),
        (
        ...
        (("clarke1880", 5), Ellipsoid(a=6378249.145, f=1.0 / 293.465, name="Clarke 1880")),
        (("everest1830", 6), Ellipsoid(a=6377276.345, f=1.0 / 300.8017, name="Everest 1830")),
        (
            ("everestmodified", 7),
        ...
        (("hough1956", "hough", 10), Ellipsoid(a=6378270.0, f=1.0 / 297, name="Hough 1956")),
        (
        ...
        (("nwl-9d", "wgs66", 13), Ellipsoid(a=6378145.0, f=1.0 / 298.25, name="NWL-9D / WGS 66")),
        (
        ...
        (("ngo1948", 20), Ellipsoid(a=6377492.0176, f=1 / 299.15281285, name="NGO1948")),
    ],
)
...
def test_mdot():
    ...
    b = -a
    t = np.concatenate([np.dot(a[..., i], b[..., i])[:, :, None] for i in range(2)], axis=2)
    tm = mdot(a, b)

    assert_allclose(t, tm)

    t1 = np.concatenate(
        [np.dot(a[..., i], b[:, 0, 0][:, None])[:, :, None] for i in range(2)], axis=2
    )
