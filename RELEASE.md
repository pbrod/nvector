...
> Releases are created from Git tags. A package is only published to PyPI when

> a version tag beginning with `v` is pushed and all CI checks pass.
...
| `ci-test.yml` | Run tests and validate builds | Push, Pull Request, Schedule |
...
A release cannot be published unless all CI jobs pass.
...
1\. Runs the CI workflow.

2\. Builds the package using PDM.
...
point = nv.GeoPoint.from\_degrees(latitude=60.0, longitude=5.0)
